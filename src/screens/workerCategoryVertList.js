...
class WorkerCategoriesVertList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  renderCategories = ({ item, index }) => {
    return (
      <TouchableOpacity
        onPress={() => {
          this.props.navigation.navigate("CategoriesList", { category: item, latitude: this.props.navigation.state.params.latitude, longitude: this.props.navigation.state.params.longitude, userid: this.props.navigation.state.params.userid });
        }}
          ...
          <View style={{ flexDirection: "row", }}>
            <View
              ...
              <Image
                source={{ uri: item.url }}
                resizeMode={"contain"}
                style={{ height: verticalScale(30), width: verticalScale(30), tintColor: "white" }}
              />
            </View>
            <View style={{ paddingHorizontal: scale(10), justifyContent: "center" }}>
              <Text numberOfLines={2} ellipsizeMode='tail' style={{ fontSize: scale(16), fontFamily: "Poppins-SemiBold", color: mainColor, width: scale(180) }}>{strings(item.name)}</Text>
              <Text numberOfLines={1} ellipsizeMode='tail' style={{ fontSize: scale(12), fontFamily: "Poppins-Light", color: "#B5B5B5", width: scale(180) }}>
                {item.provider1
      ...
      if (itemData.indexOf(textData) > -1) {
        ...
        if (item.services && item.services.length > 0)
          for (let i = 0; i < item.services.length; i++) {
            ...
            const catName = service.categoryName ? service.categoryName.toUpperCase() : "";
            if (catName.indexOf(textData) > -1) {
              isExists = true
              break
            } else {
              if (service.subCategories && service.subCategories.length > 0)
                for (let j = 0; j < service.subCategories.length; j++) {
                  const subCat = service.subCategories[j];
                  const subCatName = subCat.name ? subCat.name.toUpperCase() : "";
                  if (subCatName.indexOf(textData) > -1) {
                    isExists = true
                    break
                  }
                }
    ...
    return (
      <View style={{ flex: 1 }}>
        <Header
          ...
          centerComponent={
            <View height={40} width={scale(230)} borderRadius={20} backgroundColor="#F4F4F4" flexDirection="row">
              <View justifyContent="center" marginLeft={scale(15)} height={40}>
                <Icon
                  type="FontAwesome"
                  onPress={() => {
                  }}
        ...
        <View
          ...
          <Text
            ...
            {this.state.filtertext == "" ? strings("Categories") : strings("Worker")}
          </Text>
        </View>
        <View marginTop={0} height={verticalScale(10)} />
        {this.state.fetching ? (
...
export default connect(MapStateToProps, {})(WorkerCategoriesVertList);
